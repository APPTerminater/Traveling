...
public class EditDiaryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chosenDate = getIntent.getStringExtra("selectdate");
        showdate.setText(chosenDate);
        cityname = getIntent.getStringExtra("cityname");
        city.setText(cityname);

        //数据库
        dao = new DiaryInfoDao(EditDiaryActivity.this);
        DiaryInfo diaryInfo = dao.alterData(nickname,selectTime);
    ...
    public void save(View view) {
        ...
        if (dao.alterData(nickname,selectTime).id == -1) {
            if (TextUtils.isEmpty(diaryInfo.title) || TextUtils.isEmpty(money.getText().toString().trim())
                ...
                long addLong = dao.addData(diaryInfo);
                if (addLong == -1) {
            ...
            if  (TextUtils.isEmpty(diaryInfo.title) || TextUtils.isEmpty(money.getText().toString().trim())
                ...
                int count = dao.updateData(dao.alterData(nickname,selectTime).id,diaryInfo);
                if (count == -1) {
    ...
    public void backToDate() {
        ...
        dateintent.putExtra("selectdate", chosenDate);
