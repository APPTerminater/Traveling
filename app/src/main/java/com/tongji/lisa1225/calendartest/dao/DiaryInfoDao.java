...
public class DiaryInfoDao {
    ...
    public int updateStep(long time,String nickname,int step) {
        ...
        contentValues.put(DiaryInfo.KEY_nickname,nickname);
        contentValues.put(DiaryInfo.Key_step,step);
        int updateResult = sqLiteDatabase.update(DiaryInfo.TABLE, contentValues, "time=? and nickname=?",
                new String[]{String.valueOf(time),nickname});
        sqLiteDatabase.close();
        return updateResult;
    }
    ...
    public int deleteData(int id,String nickname) {
        SQLiteDatabase sqLiteDatabase = mydBHelper.getWritableDatabase();
        int deleteResult = sqLiteDatabase.delete(DiaryInfo.TABLE, "id=? and nickname=?",
                new String[]{String.valueOf(id),nickname});
        sqLiteDatabase.close();
        return deleteResult;
    }
    ...
    public int updateData(int id,DiaryInfo diaryInfo) {
        ...
        contentValues.put(DiaryInfo.KEY_destination,diaryInfo.destination);
        contentValues.put(DiaryInfo.Key_background,diaryInfo.background);
        int updateResult = sqLiteDatabase.update(DiaryInfo.TABLE, contentValues, "id=?",
    ...
    public DiaryInfo alterData(String nickname,long time) {
        ...
        // 查询比较特别,涉及到 cursor
        Cursor cursor = readableDatabase.query(DiaryInfo.TABLE,
                new String[]{DiaryInfo.KEY_id,DiaryInfo.KEY_temperature,DiaryInfo.KEY_cost,
                    DiaryInfo.KEY_title,DiaryInfo.KEY_text,DiaryInfo.KEY_font,DiaryInfo.KEY_bold,
                    DiaryInfo.KEY_size,DiaryInfo.KEY_color,
    ...
    public List<DiaryInfo> alterData(String nickname) {
        ...
        SQLiteDatabase readableDatabase = mydBHelper.getReadableDatabase();
        // 查询比较特别,涉及到 cursor
        Cursor cursor = readableDatabase.query(DiaryInfo.TABLE, new String[]
            {DiaryInfo.KEY_id,DiaryInfo.KEY_temperature,DiaryInfo.KEY_cost,
